...
function draw() {
  ...
  // console.log("Abs value "+abs(2))
  // console.log("trex:"+trex.y);
  // console.log("height:"+height/1.1);
  if (gameState===PLAY){
    score = score + Math.round(getFrameRate()/60);
    ground.velocityX = -(6 + 3*score/100);
  
    if(((keyDown("space"))||(touches.length>0)) && trex.y >= height/1.1) {
      trex.velocityY = -12;
      touches=[];
    }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    ...
    cloud.y = Math.round(random(50,height-trex.height-20));
